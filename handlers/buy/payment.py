...
async def handle_successful_payment(payment_id: int, user_id: int, server_id: int, message_id: int = None, subscription_id: int = None, is_renewal: bool = False):
    ...
    try:
        ...
        # При первой покупке (is_renewal=False) показываем только кнопку "Инструкции"
        # При продлении (is_renewal=True) показываем кнопки "Продлить" и "Назад к профилю" (без инструкций)
